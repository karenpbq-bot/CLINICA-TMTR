...
Incluye: información general, anamnesis, signos vitales,
antecedentes, odontograma diagnóstico interactivo y observaciones.
"""
...
from modulo_pacientes import ESTADOS_DIENTE, CARAS, DIENTES_ADULTO
...
# ── Widget de diente para la cuadrícula del odontograma ───────────────────

class _DienteDiag(ft.Column):
    """
    Diente compacto para la cuadrícula del odontograma.
    Clic en cualquier superficie → selecciona la pieza.
    Indicador azul = tiene diagnóstico.
    """
...
# ── Panel de detalle para una pieza seleccionada ──────────────────────────

class _DetalleDiente(ft.Column):
    """
    Panel derecho: muestra superficies interactivas (tamaño grande),
    campo de diagnóstico y botón guardar para la pieza seleccionada.
    """
        ...
        self._estado_resumen.controls = [
            ft.Container(
                content=ft.Row(controls=[
                    ...
                    ft.Text(
                        f"{cara.capitalize()}: {ESTADOS_DIENTE[st]['label']}",
                        size=11,
                    ),
                ], spacing=5),
    ...
    def _construir(self):
        ...
        self.controls = [
            ...
            ft.Container(
                content=ft.Column(
                    controls=self._estado_resumen.controls,
                    spacing=4,
                ),
            ),
...
class OdontogramaDiagnosticoView(ft.Column):
    """
    Odontograma interactivo con panel de diagnóstico por pieza.
    - Izquierda: cuadrícula de 32 dientes (2 filas), clic → seleccionar pieza
    - Derecha: panel de detalle con superficies interactivas + diagnóstico
    Guarda superficies en tabla `odontograma` y diagnósticos en
    `historia_clinica.diagnostico_dental`.
    """
    ...
    def _actualizar_panel(self):
        ...
        self._panel.content = _DetalleDiente(
            ...
            caras_estado=caras,
            diagnostico=diag,
            on_guardar=self._guardar_pieza,
        )
        if self._panel.page:
            self._panel.update()
    ...
    def _guardar_pieza(self, numero: int, caras: dict, diagnostico: str):
        # 1. Guardar superficies en tabla odontograma
        try:
        ...
        # 2. Guardar diagnóstico en historia_clinica.diagnostico_dental
        ...
        # 3. Actualizar indicador del diente en la cuadrícula
    ...
    def _construir_layout(self):
        cuadricula = ft.Container(
            content=ft.Column(controls=[
                ...
                ft.Divider(height=6),
                ft.Text("Leyenda de estados (clic en superficie del panel derecho):",
                        size=10, color="#757575"),
                self._leyenda(),
                ft.Container(height=4),
                ft.Row(controls=[
                    ft.Container(width=10, height=10, bgcolor="#1565C0", border_radius=2),
                    ft.Text("= tiene diagnóstico registrado", size=10, color="#424242"),
                ], spacing=5),
            ], spacing=6),
            border=ft.border.all(1, "#E0E0E0"),
...
# ── Vista principal de Historia Clínica ───────────────────────────────────

class HistoriaClinicaView(ft.Column):
    """
    Formulario completo de Historia Clínica Odontológica:
    Información general · Anamnesis · Signos vitales ·
    Antecedentes · Odontograma diagnóstico · Observaciones.
    """
    ...
    def _construir(self):
        ...
        # ── 4. Antecedentes (checkboxes 2 columnas) ───────────────────────
        self._cb_map = {}
        for key, _ in ANTECEDENTES:
            self._cb_map[key] = ft.Checkbox(
                value=bool(ant.get(key, False)), label="",
            )
        ...
        lbl_btn = ("Actualizar Historia Clínica" if existe
                   else "Guardar Historia Clínica")
    ...
    def _guardar(self, e):
        ...
        # Leer diagnósticos actuales del odontograma (incluye los guardados en la sesión)
        diagnostico_dental = (
            self._odontograma_view.diagnosticos
            if self._odontograma_view else {}
        )
